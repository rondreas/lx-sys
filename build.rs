...
fn main() {
    ...
    let path = std::path::Path::new("wrapper.h");

    if !path.exists() {
        // We then attempt to create the wrapper.h file,
        let mut header = std::fs::File::create(path).expect("Failed to create 'wrapper.h'");
    ...
    let bindings = bindgen::Builder::default()
        ...
        // bindings for.
        .header("wrapper.h")
        .allowlist_type("ILxModule")
        .allowlist_type("ILxModuleID")
